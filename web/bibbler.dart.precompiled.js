...
$$.BoundClosure$0 = [F, {"": "BoundClosure;_self,_target,_receiver,__js_helper$_name",
  call$0: function() {
    return this._target.call(this._self);
  }
}];

$$.BoundClosure$i1 = [F, {"": "BoundClosure;_self,_target,_receiver,__js_helper$_name",
  call$1: function(p0) {
...
["bibbler", "bibbler.dart", , F, {
main: function() {
  F.GameManager$(4);
},

GameManager: {"": "Object;size,startTiles,inputManager,actuator,storageManager,grid,score,bestScore,over,won,continuing,scrabbleLetters,currentTiles,words",
  handleError$1: function(error) {
    return window.alert("Error initializing words: " + H.S(error));
  },
  get$handleError: function() {
    return new H.BoundClosure$1(this, F.GameManager.prototype.handleError$1, null, "handleError$1");
  },
  getStringsReady$0: function() {
    return W.HttpRequest_getString("src/words.json", null, null).then$1(new F.GameManager_getStringsReady_closure(this)).catchError$1(this.get$handleError());
  },
  submit$0: function(_) {
    var t1, sum, tile, t2, t3, t4;
    if (this.makesWord$1(this.currentTiles) === true) {
      for (t1 = this.currentTiles, t1 = new H.ListIterator(t1, t1.length, 0, null), sum = 0; t1.moveNext$0();) {
        tile = t1._dev$_current;
        t2 = this.grid.cells;
        ...
        if (t2 >>> 0 !== t2 || t2 >= t4.length)
          throw H.ioore(t4, t2);
        t4[t2] = null;
        t3 = t3.get$value(tile);
        if (typeof t3 !== "number")
          throw H.iae(t3);
        sum += t3;
      }
      t1 = this.score;
      t2 = this.currentTiles.length;
      if (typeof t1 !== "number")
        throw t1.$add();
      this.score = t1 + sum * t2;
    }
    this.currentTiles = [];
    this.addRandomTile$0();
    this.move$1(2);
  },
  get$submit: function(_receiver) {
    return new H.BoundClosure$i0(this, F.GameManager.prototype.submit$0, _receiver, "submit$0");
  },
  makesWord$1: function(currentTiles) {
    var t1, letterList;
    for (t1 = new H.ListIterator(currentTiles, currentTiles.length, 0, null), letterList = ""; t1.moveNext$0();)
      letterList += t1._dev$_current.get$$char().toLowerCase();
    return J.contains$1$asx(this.words, letterList);
  },
  restart$0: function() {
    var t1 = J.get$classes$x(this.actuator.messageContainer);
    t1.remove$1(t1, "game-won");
    t1.remove$1(t1, "game-over");
    this.setup$0();
  },
  get$restart: function() {
    return new F.BoundClosure$0(this, F.GameManager.prototype.restart$0, null, "restart$0");
  },
  keepPlaying$0: function() {
    this.continuing = true;
    var t1 = J.get$classes$x(this.actuator.messageContainer);
    t1.remove$1(t1, "game-won");
    t1.remove$1(t1, "game-over");
  },
  get$keepPlaying: function() {
    return new F.BoundClosure$0(this, F.GameManager.prototype.keepPlaying$0, null, "keepPlaying$0");
  },
  setup$0: function() {
    this.grid = F.Grid_Grid(this.size);
    this.score = 0;
    this.over = false;
    this.won = false;
    this.continuing = false;
    this.addStartTiles$0();
    this.move$1(2);
  },
  addStartTiles$0: function() {
    var x, t1, tile, t2;
    for (x = 0; t1 = this.size, x < t1; ++x) {
      tile = F.Tile$(new F.Cell(x, t1 - 2), this.randomLetter$0());
      t1 = this.grid.cells;
      t2 = tile.x;
      ...
      if (t1 >>> 0 !== t1 || t1 >= t2.length)
        throw H.ioore(t2, t1);
      t2[t1] = tile;
      tile = F.Tile$(new F.Cell(x, this.size - 3), this.randomLetter$0());
      t1 = this.grid.cells;
      t2 = tile.x;
      if (t2 >>> 0 !== t2 || t2 >= t1.length)
        throw H.ioore(t1, t2);
      ...
      if (t1 >>> 0 !== t1 || t1 >= t2.length)
        throw H.ioore(t2, t1);
      t2[t1] = tile;
      tile = F.Tile$(new F.Cell(x, this.size - 4), this.randomLetter$0());
      t1 = this.grid.cells;
      t2 = tile.x;
      if (t2 >>> 0 !== t2 || t2 >= t1.length)
        throw H.ioore(t1, t2);
  ...
  addTile$1: function(position) {
    var tile, t1, t2;
    tile = F.Tile$(position, this.randomLetter$0());
    t1 = this.grid.cells;
    t2 = tile.x;
    if (t2 >>> 0 !== t2 || t2 >= t1.length)
      throw H.ioore(t1, t2);
  ...
  addRandomTile$0: function() {
    if (this.grid.availableCells$0().length !== 0) {
      var cell = this.grid.randomAvailableCell$0();
      if (J.$eq(cell.y, 0))
        this.addTile$1(cell);
      else
        this.addRandomTile$0();
  ...
  randomLetter$0: function() {
    var letterIndex, t1, letter;
    letterIndex = C.C__Random.nextInt$1(this.scrabbleLetters.length);
    t1 = this.scrabbleLetters;
    if (letterIndex < 0 || letterIndex >= t1.length)
      throw H.ioore(t1, letterIndex);
    letter = t1[letterIndex];
    this.scrabbleLetters = H.stringReplaceFirstUnchecked(t1, letter, "");
    return letter;
  },
  actuate$0: function() {
    var t1 = this.storageManager;
    if (J.$lt$n(t1.get$BestScore(), this.score))
      t1.storage.setItem(t1.BEST_SCORE_KEY, J.toString$0(this.score));
    this.bestScore = t1.get$BestScore();
    this.actuator.actuate$1(this);
  },
  prepareTiles$0: function() {
    var t1, t2, tile;
    for (t1 = this.grid.cells, t1 = new H.ListIterator(t1, t1.length, 0, null); t1.moveNext$0();)
  ...
  moveTile$2: function(tile, cell) {
    ...
    if (t1 >>> 0 !== t1 || t1 >= t2.length)
      throw H.ioore(t2, t1);
    t2[t1] = tile;
    tile.updatePosition$1(cell);
  },
  move$1: function(direction) {
    var vector, traversals, t1, cell, tile, x, t2, t3, t4;
    vector = this.getVector$1(direction);
    traversals = this.buildTraversals$1(vector);
    this.prepareTiles$0();
    for (t1 = traversals.x, t1 = new H.ListIterator(t1, t1.length, 0, null), cell = null, tile = null; t1.moveNext$0();) {
      x = t1._dev$_current;
      for (t2 = traversals.y, t2 = new H.ListIterator(t2, t2.length, 0, null); t2.moveNext$0();) {
        cell = new F.Cell(x, t2._dev$_current);
        t3 = this.grid;
        t3.toString;
        if (J.$ge$n(cell.x, 0) && J.$lt$n(cell.x, t3.size) && J.$ge$n(cell.y, 0) && J.$lt$n(cell.y, t3.size)) {
          t3 = t3.cells;
          t4 = cell.x;
          ...
          tile = null;
        if (tile != null)
          this.moveTile$2(tile, this.findFarthestPosition$2(cell, vector));
      }
    }
    if (this.grid.availableCells$0().length === 0)
      this.over = true;
  ...
  select$1: function(_, cell) {
    var tile, t1, t2;
    tile = this.grid.cellContent$1(cell);
    if (this.over !== true)
      t1 = this.won === true && this.continuing !== true;
    else
      t1 = true;
    if (t1)
      return;
    t1 = this.currentTiles;
    if (t1.length >= 1 && !this.oneAway$2(tile, C.JSArray_methods.get$last(t1)))
      return;
    if (C.JSArray_methods.contains$1(this.currentTiles, tile))
      for (; C.JSArray_methods.contains$1(this.currentTiles, tile);) {
        J.set$selected$x(C.JSArray_methods.get$last(this.currentTiles), false);
        t1 = C.JSArray_methods.get$last(this.currentTiles);
        t2 = J.getInterceptor$x(t1);
        t1 = ".tile-position-" + H.S(J.$add$ns(t2.get$x(t1), 1)) + "-" + H.S(J.$add$ns(t2.get$y(t1), 1));
        J.get$classes$x(document.querySelector(t1)).toggle$1("tile-selected");
        t1 = this.currentTiles;
        if (0 >= t1.length)
          throw H.ioore(t1, 0);
        t1.pop();
      ...
      J.set$selected$x(tile, true);
      this.actuator.toggleSelection$1(tile);
    }
  },
  get$select: function(_receiver) {
    return new F.BoundClosure$i1(this, F.GameManager.prototype.select$1, _receiver, "select$1");
  },
  oneAway$2: function(tile1, tile2) {
    var t1, t2;
    t1 = J.getInterceptor$x(tile1);
    t2 = J.getInterceptor$x(tile2);
    return J.abs$0$n(J.$sub$n(t1.get$x(tile1), t2.get$x(tile2))) <= 1 && J.abs$0$n(J.$sub$n(t1.get$y(tile1), t2.get$y(tile2))) <= 1;
  },
  getVector$1: function(direction) {
    var map = H.fillLiteralMap([0, new F.Cell(0, -1), 1, new F.Cell(1, 0), 2, new F.Cell(0, 1), 3, new F.Cell(-1, 0)], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
    return map.$index(map, direction);
  },
  buildTraversals$1: function(vector) {
    var traversals, pos, t1, listx, t2, listy;
    traversals = new F.Traversal([], []);
    for (pos = 0; pos < this.size; ++pos) {
      traversals.x.push(pos);
      traversals.y.push(pos);
    }
    t1 = J.getInterceptor$x(vector);
    if (J.$eq(t1.get$x(vector), 1)) {
      listx = [];
      for (t2 = new H.ReversedListIterable(traversals.x), H.setRuntimeTypeInfo(t2, [null]), t2 = new H.ListIterator(t2, t2.get$length(t2), 0, null); t2.moveNext$0();)
        listx.push(t2._dev$_current);
      traversals.x = listx;
    }
    if (J.$eq(t1.get$y(vector), 1)) {
      listy = [];
      for (t1 = new H.ReversedListIterable(traversals.y), H.setRuntimeTypeInfo(t1, [null]), t1 = new H.ListIterator(t1, t1.get$length(t1), 0, null); t1.moveNext$0();)
        listy.push(t1._dev$_current);
      traversals.y = listy;
    }
    return traversals;
  },
  findFarthestPosition$2: function(cell, vector) {
    var t1, cell0, t2, t3;
    t1 = J.getInterceptor$x(vector);
    do {
      cell0 = new F.Cell(J.$add$ns(cell.x, t1.get$x(vector)), J.$add$ns(cell.y, t1.get$y(vector)));
      t2 = this.grid;
      t2.toString;
      if (J.$ge$n(cell0.x, 0) && J.$lt$n(cell0.x, t2.size) && J.$ge$n(cell0.y, 0) && J.$lt$n(cell0.y, t2.size)) {
        t2 = this.grid;
        t2.toString;
        if (J.$ge$n(cell0.x, 0) && J.$lt$n(cell0.x, t2.size) && J.$ge$n(cell0.y, 0) && J.$lt$n(cell0.y, t2.size)) {
          t2 = t2.cells;
  ...
  GameManager$1: function(size) {
    this.getStringsReady$0().then$1(new F.GameManager_closure(this)).catchError$1(this.get$handleError());
  },
  static: {
GameManager$: function(size) {
  var t1 = new F.KeyboardInputManager(H.fillLiteralMap([], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null)));
  t1.listen$0();
  t1 = new F.GameManager(size, 8, t1, new F.HTMLActuator(document.querySelector(".tile-container"), document.querySelector(".score-container"), document.querySelector(".best-container"), document.querySelector(".game-message"), document.querySelector(".score-sharing"), 0), new F.LocalStorageManager("bestScore", window.localStorage), null, null, null, null, null, null, "AAAAAAAAABBCCDDDDEEEEEEEEEEEEFFGGGHHIIIIIIIIIJKLLLLMMNNNNNNOOOOOOOOPPQRRRRRRSSSSTTTTTTUUUUVVWWXYYZ", [], []);
  t1.GameManager$1(size);
  return t1;
}}

},

GameManager_closure: {"": "Closure;this_0",
  call$1: function(_) {
    var t1, t2;
    t1 = this.this_0;
    t2 = t1.inputManager;
    t2.on$2(t2, "submit", t1.get$submit(t1));
    t2.on$2(t2, "restart", t1.get$restart());
    t2.on$2(t2, "keepPlaying", t1.get$keepPlaying());
    t2.on$2(t2, "select", t1.get$select(t1));
    t1.setup$0();
  },
  $is_args1: true
},

GameManager_getStringsReady_closure: {"": "Closure;this_0",
  call$1: function(jsonString) {
    var t1 = C.C_JsonCodec.decode$1(jsonString);
    this.this_0.words = t1;
    return t1;
  },
  $is_args1: true
},

Traversal: {"": "Object;x>,y>"},

Grid: {"": "Object;size,cells,rand",
  randomAvailableCell$0: function() {
    var cells, t1;
    cells = this.availableCells$0();
    t1 = cells.length;
    if (t1 !== 0) {
      t1 = this.rand.nextInt$1(t1);
      if (t1 < 0 || t1 >= cells.length)
        throw H.ioore(cells, t1);
      return cells[t1];
    }
  },
  availableCells$0: function() {
    var availableCells, x, y, t1, t2;
    availableCells = [];
    for (x = 0; x < this.cells.length; ++x) {
      ...
      while (true) {
        ...
        if (y >= t1.length)
          throw H.ioore(t1, y);
        if (t1[y] == null)
          availableCells.push(new F.Cell(x, y));
        ++y;
      }
    }
    return availableCells;
  },
  cellContent$1: function(cell) {
    var t1, t2, t3;
    if (this.withinBounds$1(cell)) {
      t1 = this.cells;
  ...
  withinBounds$1: function(position) {
    var t1 = J.getInterceptor$x(position);
    return J.$ge$n(t1.get$x(position), 0) && J.$lt$n(t1.get$x(position), this.size) && J.$ge$n(t1.get$y(position), 0) && J.$lt$n(t1.get$y(position), this.size);
  },
  Grid$empty$1: function(size) {
    var x, t1, y;
    this.cells = P.List_List(this.size, null);
    x = 0;
    while (true) {
      t1 = this.size;
      if (typeof t1 !== "number")
  ...
  static: {
Grid_Grid: function(param) {
  return F.Grid$empty(param);
},

Grid$empty: function(size) {
  var t1 = new F.Grid(size, null, C.C__Random);
  t1.Grid$empty$1(size);
  return t1;
}}
...
HTMLActuator: {"": "Object;tileContainer,scoreContainer,bestContainer,messageContainer,sharingContainer,score",
  actuate$1: function(metadata) {
    C.Window_methods.get$animationFrame(window).then$1(new F.HTMLActuator_actuate_closure(this, metadata));
  },
  clearContainer$1: function(container) {
    var t1, result;
    for (container.toString; t1 = new W._ChildNodeListLazy(container), t1.get$isNotEmpty(t1);) {
      t1 = new W._ChildNodeListLazy(container);
      result = new W._ChildNodeListLazy(container)._this.firstChild;
      if (result == null)
        H.throwExpression(P.StateError$("No elements"));
      t1.remove$1(t1, result);
    }
  },
  addTile$1: function(tile) {
    var inner, t1, wrapper, t2, positionClass, result;
    inner = document.createElement("div", null);
    t1 = J.get$classes$x(inner);
    t1.add$1(t1, "tile-inner");
    inner.textContent = tile.get$$char();
    wrapper = document.createElement("div", null);
    wrapper.toString;
    new W._ChildNodeListLazy(wrapper)._this.appendChild(inner);
    t1 = J.getInterceptor$x(wrapper);
    t2 = t1.get$classes(wrapper);
    t2.add$1(t2, "tile");
    t2 = t1.get$classes(wrapper);
    t2.add$1(t2, "tile-1");
    t2 = tile.previousPosition;
    if (t2 != null) {
      positionClass = "tile-position-" + H.S(J.$add$ns(t2.x, 1)) + "-" + H.S(J.$add$ns(t2.y, 1));
      t1 = t1.get$classes(wrapper);
      t1.add$1(t1, positionClass);
      t1 = C.Window_methods.get$animationFrame(window);
      t2 = $.Zone__current;
      t2.toString;
      result = new P._Future(0, t2, null, null, new F.HTMLActuator_addTile_closure(this, tile, wrapper, positionClass), null, P._registerErrorHandler(null, t2), null);
      result.$builtinTypeInfo = [null];
      t1._addListener$1(result);
    } else {
      t1 = t1.get$classes(wrapper);
      t2 = new F.Cell(tile.x, tile.y);
      t1.add$1(t1, "tile-position-" + H.S(J.$add$ns(t2.x, 1)) + "-" + H.S(J.$add$ns(t2.y, 1)));
      t1.add$1(t1, "tile-new");
    }
    t1 = this.tileContainer;
    t1.toString;
    new W._ChildNodeListLazy(t1)._this.appendChild(wrapper);
  },
  stringify$1: function(position) {
    return "tile-position-" + H.S(J.$add$ns(position.x, 1)) + "-" + H.S(J.$add$ns(position.y, 1));
  },
  updateScore$1: function(newScore) {
    var t1, t2, difference, addition;
    t1 = this.scoreContainer;
    this.clearContainer$1(t1);
    t2 = this.score;
    if (typeof newScore !== "number")
      throw newScore.$sub();
    difference = newScore - t2;
    t1.textContent = newScore;
    if (difference > 0) {
      addition = document.createElement("div", null);
      t2 = J.get$classes$x(addition);
      t2.add$1(t2, "score-addition");
      addition.textContent = C.JSString_methods.$add("+", difference);
      t1.toString;
      new W._ChildNodeListLazy(t1)._this.appendChild(addition);
    }
  },
  toggleSelection$1: function(position) {
    var t1 = J.getInterceptor$x(position);
    t1 = ".tile-position-" + H.S(J.$add$ns(t1.get$x(position), 1)) + "-" + H.S(J.$add$ns(t1.get$y(position), 1));
    J.get$classes$x(document.querySelector(t1)).toggle$1("tile-selected");
  },
  message$1: function(_, won) {
    var t1, type, message, t2;
    t1 = won === true;
    type = t1 ? "game-won" : "game-over";
    message = t1 ? "You win!" : "Game over!";
    t1 = this.messageContainer;
    t2 = J.get$classes$x(t1);
    t2.add$1(t2, type);
    t1.querySelector("p").textContent = message;
  }
},

HTMLActuator_actuate_closure: {"": "Closure;this_0,metadata_1",
  call$1: function(_) {
    var t1, t2, t3, t4, cell;
    t1 = this.this_0;
    t1.clearContainer$1(t1.tileContainer);
    for (t2 = this.metadata_1, t3 = t2.grid.cells, t3 = new H.ListIterator(t3, t3.length, 0, null); t3.moveNext$0();)
      for (t4 = J.get$iterator$ax(t3._dev$_current); t4.moveNext$0();) {
        cell = t4.get$current();
        if (cell != null)
          t1.addTile$1(cell);
      }
    t1.updateScore$1(t2.score);
    t1.bestContainer.textContent = t2.bestScore;
    if (t2.over !== true)
      t3 = t2.won === true && t2.continuing !== true;
    else
      t3 = true;
    if (t3)
      t1.message$1(t1, t2.won);
  },
  $is_args1: true
},

HTMLActuator_addTile_closure: {"": "Closure;this_0,tile_1,wrapper_2,positionClass_3",
  call$1: function(_) {
    var t1, t2;
    t1 = J.get$classes$x(this.wrapper_2);
    t1.remove$1(t1, this.positionClass_3);
    t2 = this.tile_1;
    t1.add$1(t1, this.this_0.stringify$1(new F.Cell(t2.x, t2.y)));
  },
  $is_args1: true
},

KeyboardInputManager: {"": "Object;events",
  on$2: function(_, eventTag, callback) {
    var t1 = this.events;
    if (!t1.containsKey$1(t1, eventTag))
      t1.$indexSet(t1, eventTag, []);
    J.add$1$ax(t1.$index(t1, eventTag), callback);
  },
  listen$0: function() {
    var t1, t2, t3;
    t1 = new W._EventStream(document, C.EventStreamProvider_keydown._eventType, false);
    H.setRuntimeTypeInfo(t1, [null]);
    t2 = new W._EventStreamSubscription(0, t1._html$_target, t1._eventType, W._wrapZone(new F.KeyboardInputManager_listen_closure(this)), t1._useCapture);
    H.setRuntimeTypeInfo(t2, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
    t2._tryResume$0();
    t2 = document;
    t1 = C.EventStreamProvider_click._eventType;
    t2 = new W._EventStream(t2, t1, false);
    H.setRuntimeTypeInfo(t2, [null]);
    t3 = new W._EventStreamSubscription(0, t2._html$_target, t2._eventType, W._wrapZone(new F.KeyboardInputManager_listen_closure0(this)), t2._useCapture);
    H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(t2, "_EventStream", 0)]);
    t3._tryResume$0();
    t3 = this.get$restart();
    t2 = document.querySelector(".retry-button");
    t2.toString;
    ...
    t3._tryResume$0();
    t3 = this.get$restart();
    t2 = document.querySelector(".restart-button");
    t2.toString;
    t2 = new W._ElementEventStreamImpl(t2, t1, false);
    H.setRuntimeTypeInfo(t2, [null]);
    t3 = new W._EventStreamSubscription(0, t2._html$_target, t2._eventType, W._wrapZone(t3), t2._useCapture);
    H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(t2, "_EventStream", 0)]);
    t3._tryResume$0();
    t3 = this.get$keepPlaying();
    t2 = document.querySelector(".keep-playing-button");
    t2.toString;
  ...
  indexify$1: function(tag) {
    var t1, x, y;
    t1 = tag.length;
    if (26 >= t1)
      throw H.ioore(tag, 26);
    x = tag[26];
    if (28 >= t1)
      throw H.ioore(tag, 28);
    y = tag[28];
    return new F.Cell(J.$sub$n(H.Primitives_parseInt(x, null, null), 1), J.$sub$n(H.Primitives_parseInt(y, null, null), 1));
  },
  select$1: function(_, $event) {
    var t1, tag;
    t1 = J.getInterceptor$x($event);
    tag = J.get$className$x(J.get$parent$x(t1.get$target($event)));
    t1.preventDefault$0($event);
    this.emit$2("select", this.indexify$1(tag));
  },
  restart$1: function($event) {
    J.preventDefault$0$x($event);
    this.emit$1("restart");
  },
  get$restart: function() {
    return new H.BoundClosure$1(this, F.KeyboardInputManager.prototype.restart$1, null, "restart$1");
  },
  keepPlaying$1: function($event) {
    J.preventDefault$0$x($event);
    this.emit$1("keepPlaying");
  },
  get$keepPlaying: function() {
    return new H.BoundClosure$1(this, F.KeyboardInputManager.prototype.keepPlaying$1, null, "keepPlaying$1");
  },
  emit$2: function(eventTag, data) {
    var t1, t2, callback;
    t1 = this.events;
    if (t1.containsKey$1(t1, eventTag))
      for (t1 = J.get$iterator$ax(t1.$index(t1, eventTag)), t2 = data != null; t1.moveNext$0();) {
        callback = t1.get$current();
        if (t2)
          callback.call$1(data);
        else
          callback.call$0();
      }
  },
  emit$1: function(eventTag) {
    return this.emit$2(eventTag, null);
  }
},

KeyboardInputManager_listen_closure: {"": "Closure;this_0",
  call$1: function($event) {
    var t1, modifiers;
    t1 = J.getInterceptor$x($event);
    modifiers = t1.get$altKey($event) === true || t1.get$ctrlKey($event) === true || t1.get$metaKey($event) === true || t1.get$shiftKey($event) === true;
    t1 = J.get$tagName$x(W._convertNativeToDart_EventTarget($event.target));
    t1.toString;
    if (t1.toLowerCase() !== "input" && !modifiers)
      switch ($event.which) {
        case 13:
          $event.preventDefault();
          this.this_0.emit$1("submit");
          break;
        case 82:
          $event.preventDefault();
          this.this_0.emit$1("restart");
          break;
        default:
      }
  },
...
KeyboardInputManager_listen_closure0: {"": "Closure;this_1",
  call$1: function($event) {
    var t1;
    if (J.get$className$x(J.get$target$x($event)) === "tile-inner") {
      t1 = this.this_1;
      t1.select$1(t1, $event);
    }
  },
  $is_args1: true
},

LocalStorageManager: {"": "Object;BEST_SCORE_KEY,storage",
  get$BestScore: function() {
    var t1, t2;
    t1 = this.storage;
    t2 = this.BEST_SCORE_KEY;
    return t1.getItem(t2) != null ? H.Primitives_parseInt(t1.getItem(t2), null, null) : 0;
  }
},

Tile: {"": "Object;x>,y>,value>,$char<,previousPosition,selected',scrabbleValues",
  savePosition$0: function() {
    var t1 = new F.Cell(this.x, this.y);
    this.previousPosition = t1;
    return t1;
  },
  updatePosition$1: function(position) {
    this.x = position.x;
    this.y = position.y;
  },
  Tile$2: function(position, $char) {
    var t1, t2;
    t1 = this.$char;
    if (t1.length !== 1)
    ...
    this.x = position.x;
    this.y = position.y;
    t2 = this.scrabbleValues;
    this.value = t2.$index(t2, t1);
    this.previousPosition = null;
  },
  static: {
Tile$: function(position, $char) {
  var t1 = new F.Tile(null, null, null, $char, null, false, H.fillLiteralMap(["A", 1, "E", 1, "I", 1, "L", 1, "N", 1, "O", 1, "R", 1, "S", 1, "T", 1, "U", 1, "D", 2, "G", 2, "B", 3, "C", 3, "M", 3, "P", 3, "F", 4, "H", 4, "V", 4, "W", 4, "Y", 4, "K", 5, "J", 8, "X", 8, "Q", 10, "Z", 10, " ", 0], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null)));
  t1.Tile$2(position, $char);
  return t1;
}}
...
_ControllerSubscription: {"": "_BufferingStreamSubscription;_async$_controller,_async$_onData,_onError,_onDone,_zone,_state,_cancelFuture,_pending",
  ...
  get$_onPause: function() {
    return new F.BoundClosure$0(this, P._ControllerSubscription.prototype._onPause$0, null, "_onPause$0");
  },
  ...
  get$_onResume: function() {
    return new F.BoundClosure$0(this, P._ControllerSubscription.prototype._onResume$0, null, "_onResume$0");
  },
...
_BufferingStreamSubscription: {"": "Object;_async$_onData,_onError,_onDone,_zone<,_state,_cancelFuture,_pending",
  ...
  get$_onPause: function() {
    return new F.BoundClosure$0(this, P._BufferingStreamSubscription.prototype._onPause$0, null, "_onPause$0");
  },
  _onResume$0: function() {
  },
  get$_onResume: function() {
    return new F.BoundClosure$0(this, P._BufferingStreamSubscription.prototype._onResume$0, null, "_onResume$0");
  },
  _onCancel$0: function() {
  },
...
InputElement: {"": "HtmlElement;name=,value=", $isNode: true, $isEventTarget: true, "%": "HTMLInputElement"},

KeyboardEvent: {"": "UIEvent;altKey=,ctrlKey=,metaKey=,shiftKey=", "%": "KeyboardEvent"},
...
MouseEvent: {"": "UIEvent;altKey=,ctrlKey=,metaKey=,shiftKey=", "%": "DragEvent|MSPointerEvent|MouseEvent|MouseScrollEvent|MouseWheelEvent|PointerEvent|WheelEvent"},
...
TouchEvent: {"": "UIEvent;altKey=,ctrlKey=,metaKey=,shiftKey=", "%": "TouchEvent"},
...
function dart_precompiled($collectedClasses) {
  ...
  function GameManager(size, startTiles, inputManager, actuator, storageManager, grid, score, bestScore, over, won, continuing, scrabbleLetters, currentTiles, words) {
    this.size = size;
    ...
    this.scrabbleLetters = scrabbleLetters;
  ...
  function GameManager_getStringsReady_closure(this_0) {
    this.this_0 = this_0;
  }
  GameManager_getStringsReady_closure.builtin$cls = "GameManager_getStringsReady_closure";
  if (!"name" in GameManager_getStringsReady_closure)
    GameManager_getStringsReady_closure.name = "GameManager_getStringsReady_closure";
  $desc = $collectedClasses.GameManager_getStringsReady_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  GameManager_getStringsReady_closure.prototype = $desc;
  function Traversal(x, y) {
    this.x = x;
    this.y = y;
  }
  Traversal.builtin$cls = "Traversal";
  if (!"name" in Traversal)
    Traversal.name = "Traversal";
  $desc = $collectedClasses.Traversal;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Traversal.prototype = $desc;
  Traversal.prototype.get$x = function(receiver) {
    return this.x;
  };
  Traversal.prototype.get$y = function(receiver) {
    return this.y;
  };
  function Grid(size, cells, rand) {
    this.size = size;
    this.cells = cells;
    this.rand = rand;
  }
  ...
  if ($desc instanceof Array)
    $desc = $desc[1];
  Cell.prototype = $desc;
  Cell.prototype.get$x = function(receiver) {
    return this.x;
  };
  Cell.prototype.get$y = function(receiver) {
    return this.y;
  };
  function HTMLActuator(tileContainer, scoreContainer, bestContainer, messageContainer, sharingContainer, score) {
    this.tileContainer = tileContainer;
    this.scoreContainer = scoreContainer;
    this.bestContainer = bestContainer;
    this.messageContainer = messageContainer;
    this.sharingContainer = sharingContainer;
    this.score = score;
  }
  HTMLActuator.builtin$cls = "HTMLActuator";
  if (!"name" in HTMLActuator)
    HTMLActuator.name = "HTMLActuator";
  $desc = $collectedClasses.HTMLActuator;
  if ($desc instanceof Array)
    $desc = $desc[1];
  HTMLActuator.prototype = $desc;
  function HTMLActuator_actuate_closure(this_0, metadata_1) {
    this.this_0 = this_0;
    this.metadata_1 = metadata_1;
  }
  HTMLActuator_actuate_closure.builtin$cls = "HTMLActuator_actuate_closure";
  if (!"name" in HTMLActuator_actuate_closure)
    HTMLActuator_actuate_closure.name = "HTMLActuator_actuate_closure";
  $desc = $collectedClasses.HTMLActuator_actuate_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  HTMLActuator_actuate_closure.prototype = $desc;
  function HTMLActuator_addTile_closure(this_0, tile_1, wrapper_2, positionClass_3) {
    this.this_0 = this_0;
    this.tile_1 = tile_1;
    this.wrapper_2 = wrapper_2;
    this.positionClass_3 = positionClass_3;
  }
  HTMLActuator_addTile_closure.builtin$cls = "HTMLActuator_addTile_closure";
  if (!"name" in HTMLActuator_addTile_closure)
    HTMLActuator_addTile_closure.name = "HTMLActuator_addTile_closure";
  $desc = $collectedClasses.HTMLActuator_addTile_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  HTMLActuator_addTile_closure.prototype = $desc;
  function KeyboardInputManager(events) {
    this.events = events;
  }
  KeyboardInputManager.builtin$cls = "KeyboardInputManager";
  if (!"name" in KeyboardInputManager)
    KeyboardInputManager.name = "KeyboardInputManager";
  $desc = $collectedClasses.KeyboardInputManager;
  if ($desc instanceof Array)
    $desc = $desc[1];
  KeyboardInputManager.prototype = $desc;
  function KeyboardInputManager_listen_closure(this_0) {
    this.this_0 = this_0;
  }
  KeyboardInputManager_listen_closure.builtin$cls = "KeyboardInputManager_listen_closure";
  if (!"name" in KeyboardInputManager_listen_closure)
    KeyboardInputManager_listen_closure.name = "KeyboardInputManager_listen_closure";
  $desc = $collectedClasses.KeyboardInputManager_listen_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  KeyboardInputManager_listen_closure.prototype = $desc;
  function KeyboardInputManager_listen_closure0(this_1) {
    this.this_1 = this_1;
  }
  KeyboardInputManager_listen_closure0.builtin$cls = "KeyboardInputManager_listen_closure0";
  if (!"name" in KeyboardInputManager_listen_closure0)
    KeyboardInputManager_listen_closure0.name = "KeyboardInputManager_listen_closure0";
  $desc = $collectedClasses.KeyboardInputManager_listen_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  KeyboardInputManager_listen_closure0.prototype = $desc;
  function LocalStorageManager(BEST_SCORE_KEY, storage) {
    this.BEST_SCORE_KEY = BEST_SCORE_KEY;
    this.storage = storage;
  }
  LocalStorageManager.builtin$cls = "LocalStorageManager";
  if (!"name" in LocalStorageManager)
    LocalStorageManager.name = "LocalStorageManager";
  $desc = $collectedClasses.LocalStorageManager;
  if ($desc instanceof Array)
    $desc = $desc[1];
  LocalStorageManager.prototype = $desc;
  function Tile(x, y, value, $char, previousPosition, selected, scrabbleValues) {
    this.x = x;
    this.y = y;
    this.value = value;
    this.$char = $char;
  ...
  Tile.prototype.get$value = function(receiver) {
    return this.value;
  };
  Tile.prototype.get$$char = function() {
    return this.$char;
  };
  Tile.prototype.set$selected = function(receiver, v) {
